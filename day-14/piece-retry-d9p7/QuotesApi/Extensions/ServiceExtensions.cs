...
using QuotesApi.Features.Quotes.Commands;
using QuotesApi.Features.Quotes.Queries;
...
public static class ServiceExtensions
{
        ...
        IHostEnvironment environment)
    {
        ...
        // ── Outbound HttpClient + resilience pipeline ─────────────────────────
        // Named client "my-service" can be resolved with IHttpClientFactory.CreateClient("my-service").
        // The resilience handler wraps every outbound call with retry → circuit breaker → timeout
        // (executed inside-out: timeout is the innermost strategy, applied per attempt).
        services.AddHttpClient("my-service")
            .AddResilienceHandler("default", (b, context) =>
            {
                // Resolve a logger from DI for the retry callback so retries are never silent.
                ...
                b.AddRetry(new HttpRetryStrategyOptions
                {
                    ...
                    OnRetry = args =>
                    {
                        ...
                        logger.LogWarning(
                            exception,
                            "HTTP retry {AttemptNumber} for {RequestUri} after {DelayMs}ms — status: {StatusCode}, error: {ErrorMessage}",
