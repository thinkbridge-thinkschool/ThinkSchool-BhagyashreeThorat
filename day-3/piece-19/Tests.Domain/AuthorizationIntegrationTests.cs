...
public class AuthorizationIntegrationTests : IClassFixture<AuthTestFactory>
{
    ...
    // 4b. Valid token that is expired → authentication fails → 401
    //
    //     Distinct from 403: the token is structurally valid (correct signature,
    //     correct issuer/audience) but the lifetime check rejects it.
    //     ValidateLifetime = true with ClockSkew = 30s means a token that expired
    //     more than 30 seconds ago yields 401, not 403.
    [Fact]
    public async Task PostQuotes_ExpiredAccessToken_Returns401()
    {
        var expiredToken = BuildExpiredToken(1, "admin@quotes.com");
    ...
    // Builds a structurally valid JWT whose lifetime has already passed.
    // The token carries the correct signature so InternalJwtScheme recognises it,
    // but ValidateLifetime = true rejects it → middleware challenges → 401.
    private static string BuildExpiredToken(int userId, string email)
    {
        ...
        var token = new JwtSecurityToken(
            ...
            [
                ...
                new Claim(AppClaimTypes.Scope, "quotes.write"),
            ],
            notBefore: DateTime.UtcNow.AddMinutes(-30),
            expires: DateTime.UtcNow.AddMinutes(-15), // expired 15 min ago — past the 30s clock skew
