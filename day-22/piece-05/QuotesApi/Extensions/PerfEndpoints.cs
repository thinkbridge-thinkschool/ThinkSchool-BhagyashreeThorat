...
public static class PerfEndpoints
{
    public static IEndpointRouteBuilder MapPerfEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // ── DAY-21 CACHED: same summary, behind HybridCache ───────────────────
        // Identical contract to /summary (same DTO, same data) — the ONLY
        // difference is that the expensive EF query is wrapped in
        // HybridCache.GetOrCreateAsync. Compare this endpoint against /summary
        // under load to see the DB-load drop and the p99 improvement.
        //
        // How a request flows:
        //   1. Check L1 (in-memory). Hit  → return, no DB, no Redis.
        //   2. Check L2 (Redis).     Hit  → return, back-fill L1, no DB.
        //   3. Miss on both          → run the factory (the DB query) ONCE,
        //                              store into L1 + L2, return.
        //
        // STAMPEDE PROTECTION lives in step 3: if 200 requests miss the same key
        // simultaneously, HybridCache executes the factory a single time and
        // every other caller awaits that same result. We prove it with the
        // CacheMetrics counter — `dbQueries` is incremented INSIDE the factory,
        // so it rises by 1 even when 200 requests arrive together.
        group.MapGet("/summary-cached", async (
            AppDbContext db,
            HybridCache cache,
        ...
        {
            using var activity = QuotesApiActivitySource.Source
                .StartActivity("authors.summary.cached");
            ...
            var result = await cache.GetOrCreateAsync(
                ...
                async token =>
                {
                    // This delegate runs ONLY on a genuine cache miss. Every
                    // increment here is one real database round-trip — exactly
                    // the load we are trying to shed.
        ...
        // ── DAY-21 STATS: read the hit-rate / DB-load tally ───────────────────
        group.MapGet("/cache-stats", (CacheMetrics metrics) =>
        {
            var requests = metrics.Requests;
            var dbQueries = metrics.DbQueries;
            var served = requests - dbQueries;
            var hitRate = requests == 0 ? 0d : (double)served / requests;
            ...
            return Results.Ok(new
            {
                ...
                hitRatePercent = Math.Round(hitRate * 100, 2),
            });
        });

        // ── DAY-21 RESET: evict the key + zero the counters ───────────────────
        // Gives each experiment a cold start: the next request is a guaranteed
        // miss, so we can demonstrate the stampede behaviour deterministically.
        group.MapPost("/cache-reset", async (HybridCache cache, CacheMetrics metrics, CancellationToken ct) =>
        {
            await cache.RemoveAsync("authors:summary", ct);
            metrics.Reset();
            return Results.Ok(new { message = "cache key evicted and counters reset" });
        });
