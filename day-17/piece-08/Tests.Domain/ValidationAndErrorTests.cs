...
public class ValidationAndErrorTests : IAsyncDisposable
{
    ...
    // ── B2: Domain validation (PUT Quote) ────────────────────────────────── //

    // 6b — PUT quote with empty author → Quote.Update returns Fail → 400
    [Fact]
    public async Task PutQuote_WithEmptyAuthor_Returns400()
    {
    ...
    // 6c — PUT quote with text too long → Quote.Update returns Fail → 400
    [Fact]
    public async Task PutQuote_WithTextTooLong_Returns400()
    {
        var quoteId = await DbSeeder.SeedQuoteAsync(_factory.Services,
            "Original Author", "Original text.");
        ...
        var tooLongText = new string('T', 1001);
        var response    = await client.PutAsJsonAsync($"/api/quotes/{quoteId}",
            new { author = "Valid Author", text = tooLongText });
